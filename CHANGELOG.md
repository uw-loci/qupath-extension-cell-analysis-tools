...
- **"Reduce features with PCA before clustering"** in the Run Clustering dialog, for panels
  ...
  - **Off by default, and chosen per run before it starts.** It is fully repeatable -- the
    same cells, settings and seed give the same clusters every time -- but the result differs
    from the same run with it off, so two runs differing only in this setting are not
    comparable. That is a decision to make deliberately, not one to inherit from an upgrade.
    Every run records whether it ran, in the operation log, the Workflow tab entry and
    `RUN_INFO.txt`, and a config saved before the option existed loads with it off so those
...
- **Per-component aggregate measurements** -- opt-in (default off, `qpcat.spatial.writeComponentMeasurements`). For each graph-connected component, writes `QPCAT component: size` and `QPCAT component: mean: <existing measurement>` to every cell in the component. Deliberately uses the `component` namespace to disambiguate from Leiden phenotype clusters; see the [Component vs Cluster](documentation/spatial-neighborhoods.md#component-vs-cluster-naming) worked example.
