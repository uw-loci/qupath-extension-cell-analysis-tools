...
The YAML headless batch ([HOW_TO_GUIDE section 19](HOW_TO_GUIDE.md#19-yaml-headless-batch)) is the reproducibility backbone of a QP-CAT-driven paper. A single config file captures the algorithm, parameters, image scope, statistics, and figures so the analysis can be re-run by anyone with the data.
...
| Power-user custom Groovy that doesn't fit the YAML schema | Direct Groovy scripting -- see [SCRIPTING.md](SCRIPTING.md) |

Rule of thumb: **if you'll need to run this analysis more than twice, write a YAML**. The breakeven is low because the YAML doubles as documentation.
