...
import qupath.ext.pyclustering.model.ClusteringConfig.Normalization;
import qupath.ext.pyclustering.model.PhenotypeRuleSet;
import qupath.ext.pyclustering.service.ChannelValidator;
import qupath.ext.pyclustering.service.MeasurementExtractor;
import qupath.ext.pyclustering.service.OperationLogger;
import qupath.ext.pyclustering.service.PhenotypeRuleSetManager;
...
public class PhenotypingDialog {
    ...
    public void show() {
        ...
        dialog.setTitle("PyClustering - Run Phenotyping");
    ...
    private void runPhenotyping() {
        // Validate inputs
        if (currentMarkers.isEmpty()) {
            Dialogs.showWarningNotification("PyClustering", "No markers selected.");
            return;
        }
        if (rulesList.isEmpty()) {
            Dialogs.showWarningNotification("PyClustering", "No phenotype rules defined.");
            return;
        }
        ...
        if (imageData != null) {
            long classified = imageData.getHierarchy().getDetectionObjects().stream()
                    ...
                    .count();
            if (classified > 0) {
                boolean proceed = Dialogs.showConfirmDialog("PyClustering",
    ...
    private void saveRuleSet() {
        if (currentMarkers.isEmpty()) {
            Dialogs.showWarningNotification("PyClustering", "No markers selected to save.");
            return;
        }
        if (rulesList.isEmpty()) {
            Dialogs.showWarningNotification("PyClustering", "No rules defined to save.");
            return;
        }

        var project = qupath.getProject();
        if (project == null) {
            Dialogs.showWarningNotification("PyClustering", "No project open. Open a project first.");
            return;
        }
        ...
        try {
            ...
            Dialogs.showInfoNotification("PyClustering", "Rule set '" + name + "' saved.");
            ...
            Dialogs.showErrorNotification("PyClustering", "Failed to save: " + e.getMessage());
        }
    }
    ...
    private void loadRuleSet() {
        ...
        try {
            ...
            Dialogs.showErrorNotification("PyClustering", "Failed to load: " + e.getMessage());
        }
    }
    ...
    private void applyRuleSet(PhenotypeRuleSet ruleSet) {
        ...
        if (!match.missing().isEmpty()) {
            ...
            boolean proceed = Dialogs.showConfirmDialog("PyClustering - Missing Markers",
                    sb.toString());
            if (!proceed) return;
        }
    ...
    private void validateChannels() {
        ...
        }, "PyClustering-ChannelValidation");
